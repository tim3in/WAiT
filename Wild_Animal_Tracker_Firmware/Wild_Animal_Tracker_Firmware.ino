...
const char* ssid = "xxxxx"; //Specify your WiFi SSID
const char* password = "xxxxx"; //Specify your WiFi Password

camera_fb_t * fb = NULL;
uint8_t * _jpg_buf = NULL;
char animal[20]; //Store the result of inferencing
const int pir_sensor = 13; // PIR Data Pin
...
//This function does the inferencing
void classify() {
    ...
    for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
        ...
        if(result.classification[ix].value > 0.50000) //store the result of inferencing in animal variable
   ...
   //sendind data to personal web server
...
/* ******************************************************** */
//This function class the classify()
//This function is invoked in setup() function every time the ESP32 Wakes up
void run_classifier(){
    fb = esp_camera_fb_get();
    if (!fb) {
        Serial.println("Camera capture failed");
...
/* ******************************************************* */
//This function sends data to HTTP server
void send_data_webserver(char animal[20]){
  //Open a connection to the server
      HTTPClient http;
      http.begin("http://192.168.157.130/wild_animal_tracker/upload.php");//Specify you server IP address/domain name here
...
// Turns off the ESP32-CAM white on-board LED (flash) connected to GPIO 4
 ...
 rtc_gpio_hold_dis(GPIO_NUM_4);
  ...
  esp_sleep_enable_ext0_wakeup(GPIO_NUM_13, 1); // Enables external wakeup
 
  Serial.println("Going to sleep now");
  WiFi.disconnect(); // disconnecting the wifi connection before deep sleep so stat ESP could re-establishes connection after wakeup
delay(1000);
